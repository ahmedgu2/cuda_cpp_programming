 ...
 */
__global__ void matmul(float *mat1, uint32_t nRows1, uint32_t nCols1, float *mat2, uint32_t nRows2, uint32_t nCols2, float *output)
{
...
__global__ void concat(float *x, const int len_x, float *y, const int len_y, float *output, float len_out)
{
...
__global__ void tanh_(float *mat, const int nRows, const int nCols, float *output)
{
    int row = threadIdx.y + blockDim.y * blockIdx.y;
    int col = threadIdx.x + blockDim.x * blockIdx.x;
